...
// Set the initial gradient to 1 and start back propagation
...
void Tensor::_backward() {
	if (op != nullptr && op.use_count() == 1) {
		op->backward(*this);		// Calculate the gradients without propagation
		op->tensor1._backward();	// propagate the gradients
		op->tensor2._backward();	// 
	}
	op = nullptr;		// Release op and decrease tensors’ use_count
}

Tensor Tensor::operator+(const Tensor &t) const {
	return (*shared_ptr<Operator>(new AddOperator(*this, t)))();		// eager execution
}
